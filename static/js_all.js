...
var Chart, Dashboard, activateListeners, all, averageValues, avgTempByMonth, buildCalendarJson, buildFakeGroup, cal, calculateIntervals, chart, checkAvg, dataSlice, dateDim, dayOfWeek, db, flashFloodArray, getColorIndex, getDeltas, hourChart, hurricaneSandyArray, monthChart, monthDim, monthNameDim, months, ndx, parseDate, precipReduceHack, seasonChart, seasonDim, startTime, t1, timeChart, timeSeries, yearChart,
...
calculateIntervals = function(metric) {
  var interval, max;
  max = _.max(_.pluck(_.pluck(metric.all(), 'value'), 'avg_avg'));
  interval = max / 10;
  return [interval, 2 * interval, 3 * interval, 4 * interval, 5 * interval, 6 * interval, 7 * interval, 8 * interval, 9 * interval, max];
};

months = function(currentMonth) {
  var domain, monthArray, monthsLeft;
  monthArray = ["Jan", "Feb", "Mar", "Apr", "May", "June", "July", "Aug", "Sept", "Oct", "Nov", "Dec"];
  monthsLeft = 12;
  currentMonth -= 1;
  return domain = (function() {
    ...
    while (monthsLeft -= 0) {
      ...
      if (currentMonth < 11) {
        currentMonth += 1;
        _results.push(monthArray[currentMonth]);
      } else {
        currentMonth = 0;
        _results.push(monthArray[currentMonth]);
      }
...
Dashboard = (function() {
  ...
  Dashboard.prototype.buildDimensions = function() {
    ...
    return this.dimension.monthNames = this.cf.dimension((function(_this) {
      return function(d) {
        var month;
        month = d.monthNum;
        return _this.monthArray[month];
      };
    })(this));
  };
  ...
  Dashboard.prototype.buildMonthChart = function() {
    ...
    thisChart.chartObject.dimension(this.dimension.monthNames).width(thisChart.width + 70).height(this.upperHeight).yAxisLabel(this.displayName).elasticY(true).elasticX(true).x(d3.scale.ordinal().domain(months(this.dimension.dayStamp.bottom(1)[0].Date.getMonth()))).xUnits(dc.units.ordinal).renderHorizontalGridLines(true).compose([actualValuesChart]).on("preRedraw", (function(_this) {
      return function(_chart) {
        ...
        firstDate = moment(_this.dimension.dayStamp.bottom(1)[0].Date);
        secondDate = moment(_this.dimension.dayStamp.top(1)[0].Date);
  ...
  Dashboard.prototype.buildTimeSeriesChart = function() {
    ...
    normValuesChart = dc.lineChart(thisChart.chartObject).group(metric, "Recorded " + this.displayName).valueAccessor(function(d) {
      return d.value.avg_avg;
    }).colors(['rgba(51,102,153,.5)']).interpolate('basis-open');
    thisChart.chartObject.dimension(this.dimension.monthStamp).width(thisChart.width + 30).height(this.upperHeight).yAxisLabel(this.displayName + " " + this.units).elasticY(true).x(d3.time.scale().domain([minDate, maxDate])).xUnits(d3.time.months).brushOn(false).legend(dc.legend().x(60).y(thisChart.chartObject.xAxisY() - 60).itemHeight(13).gap(5)).renderHorizontalGridLines(true).compose([actualValuesChart, normValuesChart]).renderlet((function(_this) {
      return function(_chart) {
        var act_legend, avg_legend, legend;
  ...
  Dashboard.prototype.activateListeners = function() {
    var buttonWidth, go, monthChartWidth, monthsViewable, totalWidth;
    go = this;
    timeChart = this.charts.timeSeries.chartObject;
    ...
    $(this.inputBox).keypress(function(e) {
      if (e.which === 13) {
        return $("#upperHalf .input-group button").click();
      }
    });
    ...
    return $("span.monthButton").on("click", (function(_this) {
      return function() {
        if (monthsViewable === false) {
          ...
          $("span.monthChart");
          return monthsViewable = true;
        } else {
          $("#lowerHalf div.leftCol").css("transform", "translateX(" + buttonWidth + "px)");
          $("#lowerHalf div.rightCol").css("transform", "translateX(-" + monthChartWidth + "px)");
          return monthsViewable = false;
        }
      };
    })(this));
  };
  ...
  Dashboard.prototype.loadCalendar = function() {
    var begin, cellSize, end, json, legendIntervals, metricValues, numOfMonths;
  ...
  Dashboard.prototype.renderWordCountGraph = function() {
    ...
    group.append("g").attr("transform", "translate(" + (chart.xAxisLength()) + ",0)").attr("class", "axis hack").call(yAxisRight);
...
Chart = (function() {
  ...
  Chart.prototype.updateXAxis = function(dim) {
    var monthNum;
